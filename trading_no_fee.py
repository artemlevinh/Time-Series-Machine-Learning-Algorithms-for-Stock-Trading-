...
def trading_no_fee(y_test, y_pred, rsi_series, df, y_train):
    # Initialize
    initial_cash = 10000
    initial_stocks = 10
    ...
    for i in range(len(y_test)):
        ...
        if rsi < 30 and cash_balance > current_price and predicted_price > current_price:
            action = "Buy"
            stocks_owned += 1
            cash_balance -= current_price

        elif rsi > 70 and stocks_owned > 0 and predicted_price < current_price:
            action = "Sell"
            stocks_owned -= 1
            cash_balance += current_price
    ...
    plt.title("Trading Actions & Portfolio Value (NO Transaction Fee)")
    ...
    plt.title("Trading Actions & Portfolio Value (Dates, NO Transaction Fee)")
    fig.legend(loc="upper left")
    plt.show()
